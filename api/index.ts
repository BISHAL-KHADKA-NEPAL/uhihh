...
// Create Express app instance for Vercel Serverless Function
const app = express();
...
function getGeminiApiKeys(): string[] {
  ...
  return Array.from(new Set(keys)).filter(k => Boolean(k) && k !== 'undefined' && k !== 'null');
}
...
async function executeWithKeyRotation<T>(
  ...
  if (keys.length === 0) {
    throw new Error('GEMINI_API_KEY is not configured in Vercel Environment Variables. Please open your Vercel Project Dashboard → Settings → Environment Variables, add GEMINI_API_KEY, and Redeploy.');
  }
  ...
  try {
    ...
    const prompt = `
      ...
      2. MULTIPLE-CHOICE MATRIX / GRID HANDLING:
         ...
         - Treat each grid sub-row as an individual entryId, but sample ALL sub-rows within the same grid using the SAME persona latent trait score. This guarantees consistent, natural Likert ratings across all rows of a matrix question.
...
// Generate Random Responses with Gemini
app.post('/api/generate-random-responses', async (req, res) => {
  try {
    ...
    const message = error?.message || "Failed to generate random responses";
    res.status(500).json({ error: message });
  }
});
